...
// --- INITIALISATION KONVA (On réactive draggable ici pour le Pan) ---
const container = document.getElementById('canvas-container');
let stage = new Konva.Stage({ 
    container: 'canvas-container', 
    width: container.offsetWidth, 
    height: container.offsetHeight,
    draggable: true // Réactivé pour le déplacement (Pan)
});
...
window.selectPlan = (id) => {
    ...
    Konva.Image.fromURL(plan.url, (img) => {
        ...
        img.setAttrs({ x: 0, y: 0, name: 'plan-image', listening: true });
        
        stage.scale({ x: scale, y: scale });
        stage.position({ x: 0, y: 0 }); // Reset Pan
        
        layer.add(img);
        renderPlan(); // Dessine les pièces correspondantes par dessus
        stage.batchDraw();
    }, { crossOrigin: 'anonymous' });
};

window.deletePlan = async (id, event) => {
    // Stop propagation crucial ici car imbriqué dans une liste cliquable
    if (event) event.stopPropagation();
    if (!confirm("⚠️ Supprimer définitivement ce plan et TOUTES les zones associées ?")) return;
    try {
...
// Utilisation de mousedown/move/up pour gérer le dessin CFAO
stage.on('mousedown touchstart', (e) => {
    // Si on dessine ou si on modifie, on désactive le Pan temporairement
    if (!currentPlanId || selectedPieceId) {
        stage.draggable(false);
        return; 
    }

    // On ne dessine que si on clique sur le plan (le fond) ou dans le vide
    const isBg = e.target.hasName('plan-image') || e.target === stage;
    if (!isBg) {
        stage.draggable(false); // Pas de pan si on clique sur une pièce
        return;
    }

    // Mode dessin activé : on désactive le déplacement du plan
    isDrawingPiece = true;
    stage.draggable(false); 
    ...
    currentDrawRect = new Konva.Rect({
        ...
        fill: 'rgba(37, 99, 235, 0.2)', // bleu transparent
        stroke: '#2563eb', strokeWidth: 2, dash: [5, 5], // Tirets
        name: 'temp-draw', listening: false // Ne bloque pas le move
    });
...
stage.on('mousemove touchmove', () => {
    ...
    // Calcul dimensions avec gestion dessin à l'envers
...
// Finalisation du dessin ou simple clic pour désélectionner
stage.on('mouseup touchend', (e) => {
    // Réactive le draggable par défaut
    stage.draggable(true);

    if (isDrawingPiece && currentDrawRect) {
        // Si le rectangle dessiné est minuscule (simple clic), on annule le dessin
        ...
        // Capture dimensions CFAO pour création
        lastCapturedCoords = {
            x: currentDrawRect.x(), y: currentDrawRect.y(),
            width: currentDrawRect.width(), height: currentDrawRect.height()
        };

        currentDrawRect.destroy(); // Enlève rectangle fantôme
        isDrawingPiece = false;
        currentDrawRect = null;
        
        // Mode création : ouvre éditeur vide
        openEditor(); 
        layer.draw();
        return;
    }

    // Si on a fait un simple clic dans le vide -> désélectionne
    const isBg = e.target.hasName('plan-image') || e.target === stage;
    if (isBg && !isDrawingPiece && !selectedPieceId) {
        cancelPieceEdit();
    }
});

// --- HELPER SÉLECTION EXISTANTE (Fix Requirement 2) ---
// Cette fonction est appelée directement par l'événement d'un Groupe (voir renderPlan)
window.selectExistingPiece = (id) => {
    if (isDrawingPiece) return; // Ignore si on est en train de dessiner
    ...
    openEditor(p);
    // Redessine pour montrer la bordure bleue
    renderPlan();
};

// --- HELPER OUVERTURE ÉDITEUR ---
function openEditor(data = null) {
    ...
    // Met à jour le sélecteur de séquence
    const seqSel = document.getElementById('p-edit-seq');
    if (data) {
        seqSel.value = data.seqId;
    } else {
        seqSel.value = "";
    }
}
...
window.cancelPieceEdit = () => {
    ...
    renderPlan(); // Enlève bordure sélection
};
...
window.processPiece = async () => {
    ...
    if (!nom || !seqId || !startDate) return alert("Veuillez remplir tous les champs !");

    if (selectedPieceId) {
        // MISE À JOUR
        await updateDoc(doc(db, "pieces", selectedPieceId), { nom, seqId, startDate });
    } else {
        // CRÉATION avec dimensions capturées CFAO
        if (!lastCapturedCoords) return alert("Erreur dessin zone.");
...
onSnapshot(collection(db, "plans"), (s) => {
    ...
    document.getElementById('plans-list').innerHTML = allPlans.map(p => `
        <div class="flex items-center justify-between group p-2 border rounded-lg bg-white hover:bg-slate-50 transition-all">
            <div onclick="selectPlan('${p.id}')" class="flex-1 cursor-pointer text-[11px] truncate ${currentPlanId === p.id ? 'text-blue-600 font-bold' : 'text-slate-600'}">📄 ${p.name}</div>
            <button onclick="deletePlan('${p.id}', event)" class="text-red-400 hover:text-red-600 opacity-0 group-hover:opacity-100 transition-opacity px-1">🗑️</button>
        </div>
    `).join('');
    updateMenus(); // Assure que les sélecteurs dans l'éditeur sont à jour
});
...
onSnapshot(collection(db, "sequences"), (s) => {
    ...
    document.getElementById('list-sequences').innerHTML = allSequences.map(s => `
        <div class="p-2 bg-slate-50 border rounded text-[10px] flex justify-between group shadow-sm">
            <strong>${s.name}</strong>
            <button onclick="deleteSequence('${s.id}')" class="text-red-400 opacity-0 group-hover:opacity-100 p-1">🗑️</button>
        </div>`).join('');
});
...
// --- DESSIN DES PIÈCES (Mise à jour Groupes + Texte - Fix Requirement 1 & 2) ---
function renderPlan() {
    // Nettoie tous les groupes existants
    layer.find('.p-group').forEach(g => g.destroy());
    
    // Filtre pièces du plan actuel
    allPieces.filter(p => p.planId === currentPlanId).forEach(p => {
        const isBlocked = allIssues.some(i => i.pieceId === p.id);
        const isSelected = selectedPieceId === p.id;
        const rectW = p.width || 40;
        const rectH = p.height || 30;

        // 1. CRÉATION DU GROUPE (Porteur de l'événement clic)
        const group = new Konva.Group({
            x: p.x,
            y: p.y,
            id: p.id,
            name: 'p-group',
            listening: true // Indispensable pour capter les clics
        });

        // 2. CRÉATION DU RECTANGLE (Fond)
        const rect = new Konva.Rect({
            x: 0, y: 0, // Relatif au groupe
            width: rectW,
            height: rectH,
            ...
            // name: 'p-rect', // Plus besoin de name spécifique ici
            listening: false // Le rectangle ne bloque pas le clic, il passe au groupe
        });

        // 3. CRÉATION DU TEXTE (Requirement 1)
        const text = new Konva.Text({
            x: 0, y: 0, // Relatif au groupe
            text: p.nom, // Le nom de la pièce
            fontSize: 11,
            fontFamily: 'sans-serif',
            fill: isBlocked ? 'white' : 'black', // Blanc sur rouge, noir sinon
            width: rectW, // Aligne pour centrage
            height: rectH,
            align: 'center', // Centrage horizontal
            verticalAlign: 'middle', // Centrage vertical
            listening: false // Le texte ne bloque pas le clic
        });

        // Ajoute Rect + Text au Groupe
        group.add(rect);
        group.add(text);

        // 4. ATTACHE ÉVÉNEMENT CLIC DIRECTEMENT SUR LE GROUPE (Fix Requirement 2)
        group.on('mouseup touchend', (e) => {
            // Empêche Konva d'interpréter mousedown+drag+mouseup comme un clic CFAO
            e.cancelBubble = true; 
            if (isDrawingPiece) return;
            
            // Appelle la fonction globale de sélection
            window.selectExistingPiece(p.id);
        });

        // Ajoute Groupe au Layer
        layer.add(group);
    });
    layer.draw();
}

// ... Les autres fonctions de Gantt, Séquences, Issues restent identiques ...
function renderGantt() {
    const container = document.getElementById('gantt-render');
    container.innerHTML = "<h3 class='font-bold text-xs uppercase mb-4 tracking-widest text-slate-800'>Planning de Production PERT</h3>";
    let busy = {};
    allPieces.sort((a,b) => new Date(a.startDate) - new Date(b.startDate)).forEach(piece => {
        ...
        seq.tasks.forEach((t) => {
            ...
            pHTML += `<div class='text-[9px] flex justify-between border-t py-1 opacity-80'><span>#${t.id} ${t.name} (${t.ent})</span><span class='font-bold text-blue-700'>${s.toLocaleDateString()} - ${e.toLocaleDateString()}</span></div>`;
...
window.addTaskRow = (data = null) => {
    ...
    div.innerHTML = `
        ...
        <button onclick="this.parentElement.remove()" class="text-red-500 font-bold px-1 hover:text-red-700">×</button>`;
...
window.deleteSequence = async (id) => { if (confirm("Supprimer ce modèle de travaux ?")) await deleteDoc(doc(db, "sequences", id)); };
